            ...
            <div className="bg-[#111827] border border-[#1e2d45] rounded-2xl p-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                {biz.established_year && (
                  <div>
                    <div className="text-slate-500 mb-1">Established</div>
                    <div className="text-white font-semibold">{biz.established_year}</div>
                  </div>
                )}
                {biz.employees && (
                  <div>
                    <div className="text-slate-500 mb-1">Employees</div>
                    <div className="text-white font-semibold">{biz.employees}</div>
                  </div>
                )}
                {biz.real_estate && (
                  <div>
                    <div className="text-slate-500 mb-1">Real Estate</div>
                    <div className="text-white font-semibold">{biz.real_estate}</div>
                  </div>
                )}
                {biz.reason_for_selling && (
                  <div>
                    <div className="text-slate-500 mb-1">Reason for Selling</div>
                    <div className="text-white font-semibold">{biz.reason_for_selling}</div>
                  </div>
                )}
                <div>
                  <div className="text-slate-500 mb-1">Financing</div>
                  <div className="text-white font-semibold">{biz.financing_available ? 'Yes' : 'No'}</div>
                </div>
                <div>
                  <div className="text-slate-500 mb-1">Training</div>
                  <div className="text-white font-semibold">{biz.training_available ? 'Yes' : 'No'}</div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-cyan-400/10 border border-cyan-400/30 rounded-2xl p-6 text-center">
              ...
              {biz.cash_flow && (
                <div className="text-sm text-slate-400 mt-2">
                  Cash Flow: <span className="text-cyan-400 font-bold">{fmt(biz.cash_flow)}</span>
                </div>
              )}
            </div>
            ...
            <div className="bg-[#111827] border border-[#1e2d45] rounded-2xl p-6">
              ...
              <div className="flex flex-col gap-3 text-sm">
                {biz.phone && (
                  <a href={`tel:${biz.phone}`} className="text-slate-300 hover:text-cyan-400 transition">
                    Phone: {biz.phone}
                  </a>
                )}
                {biz.email && (
                  <a href={`mailto:${biz.email}`} className="text-slate-300 hover:text-cyan-400 transition">
                    Email: {biz.email}
                  </a>
                )}
                {biz.website && (
                  <a href={biz.website} target="_blank" className="text-slate-300 hover:text-cyan-400 transition">
                    Web: {biz.website}
                  </a>
                )}
              </div>
            ...
            <div className="bg-[#111827] border border-[#1e2d45] rounded-2xl p-6">
              ...
              <div className="flex flex-col gap-3">
                ...
                />
                <textarea
                  placeholder="I am interested in this business..."
                  rows={4}
                  className="bg-[#0a0f1e] border border-[#1e2d45] rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400 text-sm"
                />
                <button className="bg-cyan-400 text-[#0a0f1e] font-black py-3 rounded-xl text-sm uppercase tracking-wide hover:bg-cyan-300 transition">
                  Send Message
                </button>
              </div>
            </div>
