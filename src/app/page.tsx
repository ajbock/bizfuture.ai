...
export default async function Home() {
  ...
  const industries = ["Restaurants & Food","Retail","Service","Health Care & Fitness","Automotive","Technology & Website","Building & Construction","Manufacturing","Pet Services","Beauty","Financial Services","Transportation & Storage","Wholesale & Distributors","Other"]
    ...
    <main className="min-h-screen bg-[#0a0f1e] text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 pt-16 pb-12 text-center">
        ...
        <h1 className="text-5xl md:text-7xl font-black text-white mb-4 leading-tight">
          Find Your Next<br /><span className="text-cyan-400">Business</span>
        </h1>
        <p className="text-lg text-slate-400 max-w-xl mx-auto mb-10">
          Browse {count?.toLocaleString()}+ businesses for sale across the USA. AI-powered matching finds the right buyer for every listing.
        </p>
        ...
        <form action="/listings" method="GET" className="bg-[#111827] border border-[#1e2d45] rounded-2xl p-6 max-w-4xl mx-auto mb-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            ...
            <button type="submit" className="bg-cyan-400 text-[#0a0f1e] font-black px-6 py-3 rounded-xl text-sm uppercase tracking-wide hover:bg-cyan-300 transition">
              Search
            </button>
          </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-16">
          {industries.slice(0, 8).map(ind => (
            <Link key={ind} href={"/listings?industry=" + encodeURIComponent(ind)}
              className="bg-[#111827] border border-[#1e2d45] text-slate-300 hover:border-cyan-400 hover:text-cyan-400 transition px-4 py-2 rounded-full text-xs font-medium">
              {ind}
            </Link>
          ))}
        </div>
      ...
      {featured && featured.length > 0 && (
        <div className="max-w-6xl mx-auto px-6 pb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {featured.map(biz => (
              <Link key={biz.id} href={"/listings/" + biz.id}
                className="bg-[#111827] border border-[#1e2d45] rounded-2xl overflow-hidden hover:border-cyan-400 transition group">
      ...
      <div className="bg-[#111827] border-t border-[#1e2d45] py-12">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center mb-12">
            <div>
              <div className="text-3xl font-black text-cyan-400">{count?.toLocaleString()}+</div>
              <div className="text-slate-500 text-sm mt-1">Active Listings</div>
            </div>
            <div>
              <div className="text-3xl font-black text-cyan-400">50</div>
              <div className="text-slate-500 text-sm mt-1">US States</div>
            </div>
            <div>
              <div className="text-3xl font-black text-cyan-400">AI</div>
              <div className="text-slate-500 text-sm mt-1">Powered Matching</div>
            </div>
            <div>
              <div className="text-3xl font-black text-cyan-400">2027</div>
              <div className="text-slate-500 text-sm mt-1">Built For</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <div className="text-center p-6">
              ...
              <p className="text-slate-400 text-sm">AI matches your listing to qualified registered buyers by budget, industry and location.</p>
            </div>
            <div className="text-center p-6">
              <div className="text-3xl mb-3">??</div>
              <h3 className="text-white font-bold mb-2">Social Auto-Posting</h3>
              <p className="text-slate-400 text-sm">Premium listings automatically posted to X and LinkedIn to reach more buyers.</p>
            </div>
          </div>

          <div className="flex flex-col md:flex-row gap-4 justify-center mb-12">
            <Link href="/listings" className="bg-cyan-400 text-[#0a0f1e] font-bold px-8 py-4 rounded-full text-sm uppercase tracking-wide hover:bg-cyan-300 transition text-center">
              Browse Businesses For Sale
            </Link>
            <Link href="/listings/new" className="border border-cyan-400 text-cyan-400 font-bold px-8 py-4 rounded-full text-sm uppercase tracking-wide hover:bg-cyan-400 hover:text-[#0a0f1e] transition text-center">
              List Your Business
            </Link>
            <Link href="/broker" className="border border-slate-500 text-slate-300 font-bold px-8 py-4 rounded-full text-sm uppercase tracking-wide hover:border-cyan-400 hover:text-cyan-400 transition text-center">
              Join as Broker
            </Link>
          </div>
